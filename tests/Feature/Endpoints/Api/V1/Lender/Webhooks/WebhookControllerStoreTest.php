...
class WebhookControllerStoreTest extends TestCase
{
    ...
    public function test_webhook_controller_store_with_invalid_type()
    {
        $this->actingAs(self::$userLenderAdmin)
            ->withHeader('X-Company', self::$company->id)
            ->postJson('/api/v1/lender/webhooks', [
    ...
    public function test_webhook_controller_store_with_missing_fields()
    {
        $this->actingAs(self::$userLenderAdmin)
            ->withHeader('X-Company', self::$company->id)
            ->postJson('/api/v1/lender/webhooks')
    ...
    public function test_webhook_controller_store_webhook_count_can_not_be_more_the_type_limit()
    {
        $limit = 5;
        config()->set('webhook-server.limits.'.WebhookType::OrderUpdates, $limit);

        // passed till btween the limit
        for ($i = 0; $i < $limit; $i++) {
            $this->actingAs(self::$userLenderAdmin)
                ->withHeader('X-Company', self::$company->id)
                ->postJson('/api/v1/lender/webhooks', [
        ...
        // assert 422 when arrive to the limit
        $this->actingAs(self::$userLenderAdmin)
            ->withHeader('X-Company', self::$company->id)
            ->postJson('/api/v1/lender/webhooks', [
                'url' => self::$url,
    ...
    public function test_webhook_controller_store_lender_order_creator_can_not_access()
    {
        $this->withHeader('X-Company', self::$company->id)
