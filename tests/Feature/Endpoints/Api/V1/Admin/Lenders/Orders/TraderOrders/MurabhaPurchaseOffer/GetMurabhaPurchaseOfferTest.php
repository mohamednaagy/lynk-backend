...
class GetMurabhaPurchaseOfferTest extends TestCase
{
    ...
    public function setUp(): void
    {
        ...
        self::$getMurabhaPurchaseOfferUrl = self::BaseUrl.
            '/orders/'.
        ...
        $this->assertStatusCodeForAllRolesExceptForArea(
            ...
            function ($user, $role) {
                return $this->actingAs($user)
                    ->getJson(self::$getMurabhaPurchaseOfferUrl);
            }
        );
    }
     ...
     * @throws FileIsTooBig
     */
    public function test_get_murabha_purchase_offer_succeed(): void
    {
        $fileName = self::$traderOrder->provider.'-'.self::$traderOrder->reference.'.pdf';
        self::$traderOrder->addMedia(
            UploadedFile::fake()
                ->image($fileName)
        )->toMediaCollection(TraderOrderMediaCollection::MurabahaPurchaseOrder);
