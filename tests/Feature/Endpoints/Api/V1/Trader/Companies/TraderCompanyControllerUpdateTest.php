...
class TraderCompanyControllerUpdateTest extends TestCase
{
    ...
    {
        $this->putJson('api/v1/trader/companies/'.self::$company->id, self::$companyDetails)
            ->assertUnauthorized()
            ->assertExactJson([
    ...
    public function test_trader_company_controller_update_other_roles_can_not_access()
    {
        $this->assertLenderUserCannotAccess(function ($user, $role) {
            return $this->actingAs($user)
                ->putJson('api/v1/trader/companies/'.self::$company->id, self::$companyDetails);
        });
    }
    ...
    public function test_trader_company_controller_update_without_name_unsuccessful()
    {
        $this->actingAs(self::$trader)
            ->putJson('api/v1/trader/companies/'.self::$company->id, [
                'unique_name' => 'companyUniqueName',
                'company_cr' => '1234567891',
                'driver' => 'dmcc',
            ])
    ...
    public function test_trader_company_controller_update_without_driver_unsuccessful()
    {
        $this->actingAs(self::$trader)
            ->putJson('api/v1/trader/companies/'.self::$company->id, [
                'name' => 'testCompany',
