...
trait InteractsWithTrader
{
    ...
    public function createTraderUser(
        ...
        $userTrader = User::factory()->create(array_merge([
            'email' => $email,
            'password' => bcrypt('12345678'),
            'company_id' => $companyId,
        ], $data));

        Grantify::assignRoleToModel($userTrader, $role);
    ...
    public function assertTraderUserCannotAccess($request)
    {
        ...
        foreach ($roles as $role) {
            $user = $this->createTraderUser($company->id, $role, (string) Str::uuid().'@test.test');
