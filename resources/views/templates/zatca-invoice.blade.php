...
<div dir="{{ App::getLocale() === 'en' ? 'ltr' : 'rtl' }}">
    ...
    <div class="my-10 flex space-x-8 rtl:space-x-reverse">
        <div class="w-3/12">
            ...
            <h2 class="mb-1 font-semibold">{{ $seller->getCompanyName() }}</h2>
            <ul class="text-xs text-gray-500">
                <li>{{ $seller->getCompanyAddress()->getAddressLineOne() }}</li>
                <li>{{ $seller->getCompanyAddress()->getAddressLineTwo() }}</li>
        ...
        <div class="flex-1">
            ...
            <div class="grid grid-cols-12 gap-2 text-xs text-gray-500">
                ...
                <div class="col-span-9">{{ $order->created_at }}</div>
            </div>
        </div>
    </div>
    ...
    <div class="flex items-center justify-between">
        ...
        <div class="w-1/2">
            <div class="grid grid-cols-2 gap-4 py-3">
                ...
                <div>
                    {{ __('zatca/e-invoice.amount_with_currency', ['amount' => $order->amount->formatByDecimal()]) }}
                </div>
                <div>{{ __('zatca/e-invoice.total_before_vat') }}</div>
                <div>
                    {{ __('zatca/e-invoice.amount_with_currency', [ 'amount' => $order->amount->subtract($order->amount->multiply($seller->getVatRate()))->formatByDecimal() ]) }}
                </div>

                <div>{{ __('zatca/e-invoice.vat_total') }}</div>
                <div>
                    {{ __('zatca/e-invoice.amount_with_currency', [ 'amount' => $order->amount->multiply($seller->getVatRate())->formatByDecimal() ]) }}
                </div>
