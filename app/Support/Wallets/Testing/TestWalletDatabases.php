...
trait TestWalletDatabases
{
     ...
     * @return void
     */
    protected function bootTestWalletDatabase()
    {
    ...
    protected function ensureTestDatabaseExists($database)
    {
        ...
        try {
            $this->usingDatabase($testDatabase, function () {
                Schema::connection('wallet')->hasTable('dummy');
            });
        } catch (QueryException $e) {
            $this->usingDatabase($database, function () use ($testDatabase) {
                Schema::connection('wallet')->dropDatabaseIfExists($testDatabase);
                Schema::connection('wallet')->createDatabase($testDatabase);
            });
    ...
    {
        $original = DB::connection('wallet')->getConfig('database');
    ...
    {
        $database = DB::connection('wallet')->getConfig('database');
