...
class DMCCService
{
    ...
    public function getTTI(string $costPrice, string $profit): string
    {
        $response = $this->soap
...
//        $response = $this->soap
//            ->baseWsdl($this->prefixUrl('getPTPDocument'))
//            ->call('getPTPDocument', [
//                'ttiId' => $ttiId
//            ]);
//        $path = Storage::putFile('PTP', $file);
    ...
    {
        // upload TTIDocument for now it sample
        $response = $this->soap
        ...
        $response = $this->soap
            ...
            ->call('issueMurabahaPurchaseOffer', [
                'ttiId' => $ttiId,
                'comments' => 'create MPO',
                'ttiDocumentVersionNo' => $response->json()['versionNo'],
            ]);

        // request MPO document
        $response = $this->soap
            ->baseWsdl($this->prefixUrl('getIssueMurabahaPurchaseOfferDocumentProcess'))
            ->call('getIssueMurabahaPurchaseOfferDocumentProcess', [
                'ttiId' => $ttiId,
            ]);
//        $path = Storage::putFile('MPO', $file);

        return $response->body();
    }

    public function notificationHandler(string $ttiId): string
    {
        $response = $this->soap
