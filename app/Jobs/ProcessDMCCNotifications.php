...
class ProcessDMCCNotifications implements ShouldQueue
{
    ...
    public function handle(): void
    {
        ...
        collect($response->object()
            ...
            if (
                ...
                $this->handlePTPNotification($notification);
            } elseif (
                $notification->notificationHeaderAndEntity->notification
                ==
                'Action Required for Issue Murabaha Purchase Offer'
            ) {
                $this->handleMPONotification($notification);
            }
        });
    }

    private function handlePTPNotification($notification)
    {
        $ttiId = $notification->notificationHeaderAndEntity->notificationEntityDetails->notificationEntity[0]->entityValue;
    ...
    private function handleMPONotification($notification)
    {
        $ttiId = $notification->notificationHeaderAndEntity->notificationEntityDetails->notificationEntity[0]->entityValue;
