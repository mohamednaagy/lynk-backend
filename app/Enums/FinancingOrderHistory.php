...
final class FinancingOrderHistory extends Enum implements LocalizedEnum
{
    ...
    const CommoditySoldToMarket = 21;

    const GetOwnershipToCustomerCertificate = 22;

    const GetSellingToMarketCertificate = 23;

    const PendingDelivery = 24;

    const DeliveryConfirmed = 25;

    const DeliveryCancelled = 26;

    const OnHold = 27;

    const AttachSellConfirmationDocument = 28;

    const CreateLynkSalePledgeCertificate = 29;

    const PendingCancellation = 30;

    const SuccessCancelled = 31;

    const FailedCancelled = 32;

    const PendingDelivery = 33;
